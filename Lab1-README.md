...
3. [pingpong (easy)](#pingpong-easy)
   - [1. 实验目的](#pingpong-实验目的)
   - [2. 实验内容](#pingpong-实验内容)
   - [3. 问题解决](#pingpong-问题解决)
   - [4. 实验心得](#pingpong-实验心得)
...
父进程情况下：父进程关闭管道读端 `p[0]`，通过管道从 `fd` 读取数字，循环读取数字 `num`，如果 `num` 不能被当前素数 `prime` 整除，则将 `num` 写入新管道并传给子进程，最后关闭 `fd` 和 `p[1]` 释放资源。

![](images/1-13.png)

输入 `make qemu`，并运行 `primes` 程序。
...
### 3. 问题解决
- 缺少头文件 `#include "kernel/param.h"`，常量 `MAXARG` 没有定义导致报错。
- 没有考虑到所有 `exec` 族函数都要求参数数组以 NULL 结尾，补充 `full_argv[arg_index] = 0;` 解决问题。
